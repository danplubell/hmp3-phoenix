...
import           FastIO                 (fdToCFile, forceNextPacket, send)
import           Lexer                  (parser)
import           State
import           Style
import           Syntax
import           Tree                   hiding (Dir, File)
import qualified Tree                   (Dir, File)
import qualified UI
import           Utils
...
import qualified Data.ByteString.Char8  as P (ByteString, empty, intercalate,
                                              pack, singleton)
...
start ms = handle (\(SomeException e)  -> shutdown (Just (show e))) $ do
...
exitTime e 
         | isJust.ioErrors $ e   = False -- ignore
         | isJust.errorCalls $ e   = False -- ignore
         | otherwise               = True
-- exitTime _ = True

ioErrors::SomeException -> Maybe IOError
ioErrors e = fromException e::Maybe IOError
errorCalls::SomeException -> Maybe ErrorCall
errorCalls e   = fromException e::Maybe ErrorCall
------------------------------------------------------------------------
...
--        mv <- catch (popen (mpg321 :: String) ["-R","-"] >>= return . Just)
        mv <- catch (liftM Just (popen (mpg321::String) ["-R","-"]))
             (\(SomeException e)  -> do  
                                      warnA ("Unable to start " ++ MPG321 ++ ": " ++ show e)
                                      return Nothing)
        case mv of
            ...
            Just (r,w,e,pid) -> do
               ...
               _ <- catch (waitForProcess (pid2phdl pid)) (\(SomeException _)   -> return ExitSuccess)
...
shutdown ms =
    (do silentlyModifyST $ \st -> st { doNotResuscitate = True }
        catch writeSt (\(SomeException _) -> return ())
        withST $ \st ->
            case mp3pid st of
                Nothing  -> return ()
                Just pid -> do
...
handleMsg (S t) = do
    ...
    when (t == Stopped)     -- transition to next song
        playNext
...
genericJumpToMatch re k sel = do
        ...
        case mre of
            ...
            Just (p,forwards) -> do
                let (fs,cur,m) = k st
                    ...
                    loop fn inc n | fn n = return Nothing
                                  | otherwise = do
                                      let s = extract (fs ! n)
                                      case match p s [] of
    ...
    unless found $ putmsg (Fast (P.pack "No match found.") defaultSty) >> touchST
    return ()
------------------------------------------------------------------------
...
loadConfig = do
    ...
    when b $ do     -- otherwise used compiled-in values
        ...
        msty <- catch (readM str >>= return . Just)
                      (\(SomeException _) -> warnA "Parse error in ~/.hmp3" >> return Nothing)
        case msty of
            Nothing  -> return ()
            Just rsty -> do
