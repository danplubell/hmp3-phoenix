...
pid2phdl pid = unsafePerformIO $ mkProcessHandle pid True
{-# NOINLINE pid2phdl #-}
...
exec cmd args (pr,cw,ce) = do
        _ <- dupTo pr stdInput
        _ <- dupTo cw stdOutput      -- dup stderr too!
        _ <- dupTo ce stdError       -- dup stderr too!
...
--
isLightBg :: IO Bool
isLightBg = Control.Exception.handle (\(SomeException _)  -> return False) $ do
